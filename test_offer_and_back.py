...
def test_click_offers_and_back_buttons(driver):
    sleep(5)  # Wait for the app to load

    try:
        # Locate the "Offers" button using accessibility ID
        offers_button = driver.find_element(By.ACCESSIBILITY_ID, 'Offers')
        offers_button.click()
        
        sleep(3)  # Wait for the action to complete
        
        # Optionally verify if the Offers screen is displayed
        ...
        # Example check: Verify if the Offers button is still present
        offers_button = driver.find_element(By.ACCESSIBILITY_ID, 'Offers')
        assert offers_button.is_displayed(), "Offers button is not visible after pressing Back"
